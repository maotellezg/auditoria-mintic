/**
 * analytics.js
 * Servicio de analÃ­tica comparativa Duque vs Petro para detecciÃ³n de corrupciÃ³n.
 ...
 * - BigInt â†’ Number
 * - Date â†’ string ISO
 * - Objetos {value: x} de BigQuery (BigQueryInt, BigQueryNumeric) â†’ Number o String
...
function serializeValue(v) {
  ...
  // BigQuery devuelve nÃºmeros grandes como objetos {value: "12345678.90"}
...
// â”€â”€â”€ KPIs comparativos Duque vs Petro â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Serie mensual de gasto â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Tipos de contrato por gobierno â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Modalidades de contrataciÃ³n por gobierno â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Top 30 contratistas con scoring de riesgo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ PrestaciÃ³n de servicios por aÃ±o (nÃ³mina paralela) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Heatmap mensual (intensidad de contrataciÃ³n) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Alertas de riesgo automÃ¡ticas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function alertasRiesgo(entidadId) {
  ...
  // Verificar perÃ­odo activo (Petro)
  const totalPetro = kpis.petro_valor_total || 0;
  const nPetro = kpis.petro_n_contratos || 0;

  // 1. Alta concentraciÃ³n (top 3 > 50% del presupuesto)
  if (topC.length >= 3) {
    const top3pct = topC.slice(0, 3).reduce((s, r) => s + (r.pct_del_total || 0), 0);
    alertas.push({
      nivel: top3pct > 50 ? 'ROJO' : top3pct > 30 ? 'AMARILLO' : 'VERDE',
      tipo: 'CONCENTRACION',
      titulo: 'ConcentraciÃ³n en Contratistas',
      descripcion: `Los 3 principales contratistas recibieron el ${top3pct.toFixed(1)}% del presupuesto total (umbral crÃ­tico: 50%).`,
      valor: parseFloat(top3pct.toFixed(2)),
      umbral: 50,
    });
  }

  // 2. Alta contrataciÃ³n directa (> 60% por valor â€” perÃ­odo Petro)
  ...
  alertas.push({
    ...
    titulo: 'Alta ContrataciÃ³n Directa (Petro)',
    descripcion: `El ${pctDirectaPetro.toFixed(1)}% de los contratos en el gobierno Petro se adjudicaron por contrataciÃ³n directa (umbral crÃ­tico: 60%).`,
    valor: parseFloat(pctDirectaPetro.toFixed(2)),
    umbral: 60,
  });

  // 3. Alta contrataciÃ³n directa (Duque comparativo)
  ...
  alertas.push({
    ...
    titulo: 'Alta ContrataciÃ³n Directa (Duque)',
    descripcion: `El ${pctDirectaDuque.toFixed(1)}% de los contratos en el gobierno Duque se adjudicaron por contrataciÃ³n directa (umbral crÃ­tico: 60%).`,
  ...
  // 5. Alta prestaciÃ³n de servicios (> 40%) â€” nÃ³mina paralela
  ...
  alertas.push({
    ...
    titulo: 'Riesgo de NÃ³mina Paralela (Petro)',
    descripcion: `El ${pctPrestacionPetro.toFixed(1)}% de los contratos Petro son de prestaciÃ³n de servicios, indicador de posible nÃ³mina paralela (umbral crÃ­tico: 40%).`,
  ...
  alertas.push({
    ...
    titulo: 'Riesgo de NÃ³mina Paralela (Duque)',
    descripcion: `El ${pctPrestacionDuque.toFixed(1)}% de los contratos Duque son de prestaciÃ³n de servicios (umbral crÃ­tico: 40%).`,
...
// â”€â”€â”€ Top 50 contratos por valor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ PRESTACIÃ“N DE SERVICIOS â€” anÃ¡lisis detallado â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function prestacionServiciosDetalle(entidadId) {
  ...
  const mkTop = (docF = '') => `
    SELECT
      ...
      MAX(IFNULL(tipo_doc_proveedor, 'â€”')) AS tipo_doc,
      COUNT(*) AS n_contratos,
      SUM(SAFE_CAST(valor_del_contrato AS FLOAT64)) AS valor_total,
      AVG(SAFE_CAST(valor_del_contrato AS FLOAT64)) AS valor_promedio,
  ...
  const mkRepetidos = (docF = '') => isNitSector ? `
    SELECT
      ...
      MAX(IFNULL(t.tipo_doc_proveedor,'â€”')) AS tipo_doc,
...
// â”€â”€â”€ DIRECTOS NO PRESTACIÃ“N â€” anÃ¡lisis completo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
