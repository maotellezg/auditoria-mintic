...
export default function Chat({ onSelectDoc }) {
  ...
  const [messages, setMessages] = useState(() => {
    if (currentUser) {
      const saved = localStorage.getItem(`mintic_chat_messages_${currentUser.uid}`);
      if (saved) {
        try {
    ...
    return [
      {
        id: 'welcome',
        role: 'assistant',
        content: '¡Hola! Bienvenido a **Auditoria MinTic**, tu asistente de análisis documental inteligente. Puedo buscar, leer y extraer respuestas exactas en base a todo el corpus de resoluciones, licencias, autos e informes ambientales que han sido analizados.\n\nEscribe tu pregunta abajo para comenzar.',
        citations: [],
  ...
  useEffect(() => {
    if (currentUser && messages.length > 0) {
      localStorage.setItem(`mintic_chat_messages_${currentUser.uid}`, JSON.stringify(messages));
    }
  ...
  const clearChat = () => {
    if (window.confirm('¿Estás seguro de que deseas limpiar el historial de este chat?')) {
      ...
      if (currentUser) {
        localStorage.setItem(`mintic_chat_messages_${currentUser.uid}`, JSON.stringify(defaultMessages));
      }
    }
  };
  ...
  return (
    <div className="chat-layout" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ 
        ...
        <div>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 700, margin: 0, color: 'var(--text-main)' }}>Auditoria MinTic</h3>
